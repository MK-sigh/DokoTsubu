...
## 概要
プライベート・チャット・ログは、ユーザー登録・ログイン機能付きの簡易チャット投稿アプリです。 
書籍「スッキリわかるサーブレット＆JSP入門」に掲載された簡易なメッセージ表示アプリにCRUD処理とセキュリティに関する処理を追加し、堅牢性を高めました。
...
### 1. 認証・認可の徹底
- **IDOR（不適切な直接オブジェクト参照）対策**: チャットの更新・削除実行時、リクエストパラメータのIDだけでなく、セッション上のユーザーIDとDB上の所有者IDが一致するかをサーバーサイドで再検証します。
...
- **チャット管理**: 投稿、編集、削除（権限チェック付）、一覧表示。
- **データ永続化**: JDBCによるDB操作。

## セットアップ
1. `mvn clean package` でWARファイルを生成。
2. Tomcat等のサーブレットコンテナにデプロイ。
3. `http://localhost:8080/private-chat-log/` にアクセス。
